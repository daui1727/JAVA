...
public class BoardDAO {
	...
	public boolean boardUpdate(BoardVO vo) {
		...
		try {
			...
			getConnection();
			String sql = "select pwd from exBoard where no = ? ";
			
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, vo.getNo());
			ResultSet rs = pstmt.executeQuery();
			
			// DB에서 가져온 비밀번호를 chk_pwd에 넣고
			String chk_pwd = "";
			if(rs.next()) {
			...
			if(chk_pwd.equals(vo.getPwd())) {
				chk = true;
				sql = "update exBoard set name = ?, subject = ?, content = ? where no = ?";
	...
	public boolean boardDelete(int no, String pwd) {
		...
		try {
			...
			if(chk_pwd.equals(pwd)) {
				...
				sql = "delete from exBoard where no = ?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, no);
